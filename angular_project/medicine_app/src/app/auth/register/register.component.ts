...
export class RegisterComponent {
  ...
  dateValidator(control: AbstractControl): { [key: string]: any } | null {
    ...
    if (
      date.getFullYear() !== parseInt(control.value.split('-')[0], 10) ||
    ...
    if (this.registerForm.valid) {
      ...
      this.patientService.addPatient(this.registerForm.value).subscribe({
        next: (response: RegisterResponse) => {
          // Assuming 'RegisterResponse' is your response interface
          ...
          if (response.status_code === '0') {
            ...
            this.snackBar.open('Patient registered successfully!', 'OK', {
              duration: 4000,
            });
